...
- **Integrated QR Nav Icon**: Fixed an intuitive barcode scanning hub icon ("Ler QR") exclusively for companion standalone mobile views.
- **Responsive Schedules Bottom Bar Filtering**: Tuned the Schedules ("Horários") button in the bottom navigation to hide on narrow mobile phone dimensions to minimize menu density, while ensuring it remains visual and operational on tablet viewports and full desktop displays.
